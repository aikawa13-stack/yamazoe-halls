...
部屋マスタは、東山（１階 大会議室・１階 料理実習室・２階 和室（大）・２階 和室（小）・２階 研修室）、波多野公民館（文化伝習館）（研修室）、豊原（大研修室（ランチルーム）・研修室（トマトホール）・料理実習室）です。東山のロビー・図書室は予約対象に含めません。
...
職員管理画面からは「職員予約の新規作成」で、午前・午後・夜間の確定予約を直接登録できます。職員予約には `isStaffReservation: true` と `createdBy` を保存し、既存の予約固定ID・競合判定・休館日判定を公開予約と共通化します。職員による作成・更新・削除は `reservationAuditLogs` に記録されます。
