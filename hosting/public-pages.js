...
function startReserve() {
  ...
  form.addEventListener("submit", (event) => { event.preventDefault(); if (!form.reportValidity()) { setStatus("必須項目を入力してください。", "error"); return; } if (!validPhone()) { setStatus("電話番号は10桁以上で入力してください。", "error"); return; } const data = new FormData(form); const details = [["館", facilityLabel(facility)], ["施設", roomLabel(facility, room)], ["利用日", date], ["利用区分", slotLabel(slot)], ["氏名", data.get("customerName")], ["電話番号", data.get("phone")], ["予約者居住区分", data.get("residency") === "inside" ? "村内" : "村外"], ["利用目的", data.get("purpose")], ["備考", data.get("notes") || "—"]]; document.querySelector("#confirmation-details").replaceChildren(...details.map(([key, value]) => { const row = document.createElement("div"); row.innerHTML = `<dt>${key}</dt><dd>${value}</dd>`; return row; })); form.hidden = true; confirmation.hidden = false; });
  ...
  document.querySelector("#confirm-reservation").addEventListener("click", async () => {
    ...
    try {
      ...
      const createdAt = serverTimestamp(); const reservation = { reservationId: recordId, slotId, facility, room, date, slot, customerName: String(data.get("customerName")).trim(), phone: String(data.get("phone")).trim(), residency: String(data.get("residency")), purpose: String(data.get("purpose")).trim(), notes: String(data.get("notes")).trim(), status: "pending", createdAt }; const availability = { reservationId: recordId, slotId, facility, room, date, slot, status: "pending", createdAt };
