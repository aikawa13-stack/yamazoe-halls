...
function startReservationRibbon() {
  ...
  unsubscribeRibbon = onSnapshot(reservationsQuery, (snapshot) => {
    const active = snapshot.docs.map((item) => ({ ...item.data(), id: item.id })).filter((reservation) => ["pending", "confirmed"].includes(reservation.status)); ribbonStack.replaceChildren();
    for (const facility of facilities) { const hallReservations = active.filter((reservation) => reservation.facility === facility).sort((left, right) => (left.createdAt?.toMillis?.() || 0) - (right.createdAt?.toMillis?.() || 0)); if (!hallReservations.length && accessFacility !== "all") continue; const ribbon = document.createElement(hallReservations.length ? "button" : "span"); ribbon.className = `reservation-ribbon${hallReservations.length ? "" : " is-empty"}`; ribbon.textContent = hallReservations.length ? `${facilityLabel(facility)}：予約があります（${hallReservations.length}件）` : `${facilityLabel(facility)}：予約なし`; if (hallReservations.length) { ribbon.type = "button"; ribbon.title = "最も古い予約の詳細を開きます"; ribbon.addEventListener("click", () => { location.href = reservationDetailUrl(hallReservations[0]); }); } ribbonStack.append(ribbon); }
