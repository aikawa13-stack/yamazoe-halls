...
function showLogin(user, claims) { const login = document.querySelector("#login-panel"); const panel = document.querySelector("#staff-panel"); if (!user) { unsubscribe?.(); unsubscribe = null; unsubscribeRibbon?.(); unsubscribeRibbon = null; document.querySelector("#staff-reservation-ribbons")?.remove(); document.querySelector("#staff-user")?.replaceChildren(); login.hidden = false; panel.hidden = true; return; } if (!claims.admin || !claims.facility) { document.querySelector("#login-status").textContent = "職員権限を確認できません。再ログインしてください。"; return; } accessFacility = claims.facility; staffEmail = user.email; login.hidden = true; panel.hidden = false; const userText = document.querySelector("#staff-user"); if (userText) userText.textContent = `${user.email} としてログイン中`; if (page === "calendar") { startReservationRibbon(); startCalendar(); } else startDaily(); }
