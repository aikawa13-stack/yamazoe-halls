...
function showLogin(user, claims) { const login = document.querySelector("#login-panel"); const panel = document.querySelector("#staff-panel"); if (!user) { unsubscribeRibbon?.(); document.querySelector("#staff-reservation-ribbons")?.remove(); login.hidden = false; panel.hidden = true; return; } if (!claims.admin || !claims.facility) { document.querySelector("#login-status").textContent = "職員権限を確認できません。再ログインしてください。"; return; } accessFacility = claims.facility; staffEmail = user.email; login.hidden = true; panel.hidden = false; const userText = document.querySelector("#staff-user"); if (userText) userText.textContent = `${user.email} としてログイン中`; if (page === "calendar") { startReservationRibbon(); startCalendar(); } else startDaily(); }
...
if (page === "calendar") {
  ...
  document.body.classList.add("has-fixed-back"); const logout = document.createElement("button"); logout.type = "button"; logout.className = "page-back page-back-fixed"; logout.textContent = "ログアウト"; logout.addEventListener("click", () => signOut(auth)); document.body.append(logout);
}
