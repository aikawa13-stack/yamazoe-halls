...
let accessFacility = null; let staffEmail = null; let unsubscribe = null; let unsubscribeRibbon = null;
...
function label(status, detail = false) { return (detail ? { available: "空き", pending: "受付中", confirmed: "確定", reserved: "確定", canceled: "キャンセル済", stopped: "停止", closed: "休館日" } : { available: "空き", pending: "予約あり", confirmed: "予約あり", reserved: "予約あり", canceled: "キャンセル済", stopped: "停止", closed: "休館日" })[status] || status; }
...
function reservationRank(status) { return ({ confirmed: 3, pending: 2, canceled: 1 })[status] || 0; }
function listen(facility, month, callback) {
  const [start, end] = monthBounds(month); const records = new Map(); const reservations = new Map(); const closed = new Set(); const stoppedByRoom = new Map();
  const notify = () => callback({ records, reservations, closed, stopped: new Set([...stoppedByRoom.values()].flatMap((set) => [...set])) });
  const stops = [
    ...
    onSnapshot(query(collection(db, "reservations"), where("facility", "==", facility)), (snapshot) => { reservations.clear(); snapshot.docs.forEach((item) => { const record = item.data(); if (record.date < start || record.date > end || !record.room || !record.slot) return; const key = reservationId(facility, record.room, record.date, record.slot); const current = reservations.get(key); const newer = (record.createdAt?.toMillis?.() || 0) > (current?.createdAt?.toMillis?.() || 0); if (!current || reservationRank(record.status) > reservationRank(current.status) || (reservationRank(record.status) === reservationRank(current.status) && newer)) reservations.set(key, { ...record, id: item.id }); }); notify(); }),
...
function startReservationRibbon() {
  ...
  const facilities = accessFacility === "all" ? ["higashiyama", "hatano", "toyohara"] : [accessFacility]; const reservationsQuery = accessFacility === "all" ? query(collection(db, "reservations")) : query(collection(db, "reservations"), where("facility", "==", accessFacility));
  unsubscribeRibbon = onSnapshot(reservationsQuery, (snapshot) => {
    const pending = snapshot.docs.map((item) => ({ ...item.data(), id: item.id })).filter((reservation) => reservation.status === "pending"); ribbonStack.replaceChildren();
    for (const facility of facilities) { const hallReservations = pending.filter((reservation) => reservation.facility === facility).sort((left, right) => (left.createdAt?.toMillis?.() || 0) - (right.createdAt?.toMillis?.() || 0)); const ribbon = document.createElement(hallReservations.length ? "button" : "span"); ribbon.className = `reservation-ribbon${hallReservations.length ? "" : " is-empty"}`; ribbon.textContent = hallReservations.length ? `${facilityLabel(facility)}：受付中の予約があります（${hallReservations.length}件）` : `${facilityLabel(facility)}：受付中の予約なし`; if (hallReservations.length) { ribbon.type = "button"; ribbon.title = "最も古い受付中予約の詳細を開きます"; ribbon.addEventListener("click", () => { location.href = reservationDetailUrl(hallReservations[0]); }); } ribbonStack.append(ribbon); }
    ribbonStack.hidden = ribbonStack.childElementCount === 0;
  }, () => { ribbonStack.hidden = true; });
}

function startCalendar() {
  const facilitySelect = document.querySelector("#facility"); const calendar = document.querySelector("#calendar"); const heading = document.querySelector("#calendar-month");
  let facility = accessFacility === "all" ? (params.get("facility") || "higashiyama") : accessFacility; let month = params.get("month") || localDate(new Date()).slice(0, 7);
  facilitySelect.value = facility; facilitySelect.disabled = accessFacility !== "all";
  const render = (data) => { calendar.replaceChildren(); const [year, number] = month.split("-").map(Number); heading.textContent = `${year}年${number}月`; for (const day of ["日", "月", "火", "水", "木", "金", "土"]) { const cell = document.createElement("div"); cell.className = "weekday"; cell.textContent = day; calendar.append(cell); } const first = new Date(year, number - 1, 1); for (let i = 0; i < first.getDay(); i += 1) calendar.append(document.createElement("div")); for (let day = 1; day <= new Date(year, number, 0).getDate(); day += 1) { const date = localDate(new Date(year, number - 1, day)); const states = roomsFor(facility).flatMap((room) => slots.map((slot) => slotStatus(data, facility, room.id, date, slot.id))); const status = data.closed.has(date) ? "closed" : states.some((item) => item === "pending" || item === "confirmed") ? "pending" : states.some((item) => item === "stopped") ? "stopped" : "available"; const button = document.createElement("button"); button.type = "button"; button.className = `calendar-day is-${status}`; button.innerHTML = `<strong>${day}</strong><span class="day-marker">${label(status)}</span>`; button.addEventListener("click", () => { location.href = `/staff/daily?facility=${facility}&date=${date}`; }); calendar.append(button); } };
  const subscribe = () => { unsubscribe?.(); unsubscribe = listen(facility, month, render); };
  subscribe(); facilitySelect.addEventListener("change", () => { facility = facilitySelect.value; subscribe(); });
...
function showLogin(user, claims) {
  const login = document.querySelector("#login-panel"); const panel = document.querySelector("#staff-panel"); const publicTop = document.querySelector("#public-reservation-top");
  if (!user) {
    unsubscribe?.(); unsubscribe = null; unsubscribeRibbon?.(); unsubscribeRibbon = null; document.querySelector("#staff-reservation-ribbons")?.remove(); document.querySelector("#staff-user")?.replaceChildren();
    if (page === "calendar") { document.body.classList.add("has-fixed-back"); if (!publicTop) { const link = document.createElement("a"); link.id = "public-reservation-top"; link.className = "page-back page-back-fixed"; link.href = "/"; link.textContent = "予約フォームTOPへ"; document.body.append(link); } }
    login.hidden = false; panel.hidden = true; return;
  }
  publicTop?.remove(); if (page === "calendar") document.body.classList.remove("has-fixed-back");
  if (!claims.admin || !claims.facility) { document.querySelector("#login-status").textContent = "職員権限を確認できません。再ログインしてください。"; return; }
  accessFacility = claims.facility; staffEmail = user.email; login.hidden = true; panel.hidden = false; const userText = document.querySelector("#staff-user"); if (userText) userText.textContent = `${user.email} としてログイン中`; if (page === "calendar") { startReservationRibbon(); startCalendar(); } else startDaily();
}
